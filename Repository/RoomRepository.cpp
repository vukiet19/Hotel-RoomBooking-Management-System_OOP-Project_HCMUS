...
int RoomRepository::add(Room *room)
{
    ...
    query.bindValue(":room_number", QString::fromStdString(room->roomNumber));
...
// Cập nhật thông tin của Room trong database (bảng ListRooms)
bool RoomRepository::update(Room *room)
{
    ...
    query.prepare("UPDATE ListRooms SET room_number = :room_number, room_type = :room_type, "
...
bool RoomRepository::remove(const string &roomId)
{
    ...
    query.prepare("DELETE FROM ListRooms WHERE room_id = :room_id");
...
Room *RoomRepository::getById(const string &roomId)
{
    ...
    if (room)
    {
        ...
        room->number_people = numberPeople;
    }
