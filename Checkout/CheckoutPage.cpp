...
void CheckoutPage::loadMockBookings()
{
    ...
    QString sql = R"(
        ...
        LEFT JOIN ListRooms r ON b.room_number = r.room_number
    )";
    ...
    while (query.next())
    {
        ...
        serviceQuery.prepare("SELECT quantity, final_price FROM BookingServiceItems WHERE booking_id = :bId");
        ...
        if (serviceQuery.exec())
        {
            ...
            {
                // QDebug() << serviceQuery.value("final_price").toDouble() << '\n';
                CheckoutServicePreview item;
                item.quantity = serviceQuery.value("quantity").toInt();
                item.unitPrice = serviceQuery.value("final_price").toDouble();
                qDebug() << item.unitPrice << '\n';
...
void CheckoutPage::showConfirmDialog()
{
                              ...
                              QMessageBox::Yes | QMessageBox::No) == QMessageBox::Yes)
    {
        ...
        updateRoom.prepare("UPDATE ListRooms SET Status = 0 WHERE room_number = :roomId");
