...
void CheckoutPage::showConfirmDialog()
{
                              ...
                              QMessageBox::Yes | QMessageBox::No) == QMessageBox::Yes)
    {
        ...
        // 1. Update room status to Available (0)
        ...
        updateBooking.prepare("UPDATE Bookings SET status = 'CHECKED_OUT' WHERE id = :bId");
        updateBooking.bindValue(":bId", booking.bookingId);

        if (updateRoom.exec() && insertBill.exec() && updateBooking.exec())
        {
            db.commit();

            // Trigger Observer Events for real-time UI updates
            RoomEvent rEvent;
            rEvent.roomId = booking.roomNumber.toStdString();
            rEvent.newStatus = RoomStatus::Available;
            rEvent.timestamp = QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss").toStdString();
            HotelEventManager::instance().notifyRoomStatus(rEvent);

            BookingEvent bEvent;
            bEvent.bookingId = booking.bookingId.toInt();
            bEvent.customerName = booking.customerName.toStdString();
            bEvent.roomNumber = booking.roomNumber.toStdString();
            bEvent.newStatus = BookingStatus::CHECKED_OUT;
            bEvent.totalPrice = finalTotal;
            bEvent.timestamp = QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss").toStdString();
            HotelEventManager::instance().notifyBookingStatus(bEvent);
        ...
        {
            db.rollback();
            qDebug() << "[CHECKOUT DB ERROR]" << updateRoom.lastError().text() << insertBill.lastError().text() << updateBooking.lastError().text();
