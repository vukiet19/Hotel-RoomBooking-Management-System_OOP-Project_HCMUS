...
void MainWindowController::showInventoryTab() {
  ...
  lowStockQuery.exec("SELECT item_name, quantity, "
                     "COALESCE(minimum_quantity_required, 0) AS min_qty "
                     "FROM Inventory "
  ...
  while (lowStockQuery.next()) {
    ...
    lowStockItems << QString("• %1 (stock: %2, min required: %3)")
                         .arg(itemName)
                         .arg(qty)
                         .arg(minQty);
  }
  ...
  if (!lowStockItems.isEmpty()) {
    ...
    msgBox.setWindowTitle("⚠️ Low Stock Alert");
    msgBox.setIcon(QMessageBox::Warning);
    msgBox.setText("<b style='color:#b45309;font-size:15px;'>Items below "
                   "minimum required stock:</b>");
    msgBox.setInformativeText(lowStockItems.join("\n"));
    msgBox.setStyleSheet("QMessageBox { background-color: #fffbeb; }"
...
void MainWindowController::AddInventoryClick() {
  ...
  QString inputStyle =
      ...
      "   color: #0f172a; "
      "}"
      "QLineEdit:hover, QSpinBox:hover, QComboBox:hover { border: 2px solid "
      "#0284c7; }"
      "QLineEdit:focus, QSpinBox:focus, QComboBox:focus { border: 2px solid "
      "#0369a1; "
      "background-color: #f0f9ff; }"
      "QComboBox::drop-down { border: none; width: 28px; }"
      "QComboBox::down-arrow { image: none; border-left: 5px solid "
      "transparent; "
      "                        border-right: 5px solid transparent; "
      "                        border-top: 7px solid #0284c7; margin-right: "
      "8px; }";
  ...
  spinMinQty->setToolTip(
      "Alert triggered when quantity falls to or below this level");
  ...
  QLabel *hintLabel =
      new QLabel("💡 Alert when stock ≤ this minimum level", dialog);
...
void MainWindowController::UpdateInventoryClick() {
  ...
  int currentQty = tableInventory->item(currentRow, 3)
                       ...
                       : 0;
  double currentPrice =
      tableInventory->item(currentRow, 4)
          ? tableInventory->item(currentRow, 4)->text().toDouble()
          : 0.0;
  ...
  QString inputStyle = "QLineEdit, QSpinBox { background-color: #ffffff; "
                       "border: 2px solid #38bdf8; border-radius: 8px; "
                       "padding: 8px; font-size: 14px; color: #0f172a; }";
  ...
  QLineEdit *txtPrice =
      new QLineEdit(QString::number(currentPrice, 'f', 0), dialog);
  txtPrice->setStyleSheet(inputStyle);
  ...
  connect(btnSave, &QPushButton::clicked, [=]() {
    ...
    if (repo.updateItem(id.toInt(), spinQty->value(),
                        txtPrice->text().toDouble())) {
...
void MainWindowController::FilterInventoryClick() {
  ...
  // Filter button for low stock items
  QPushButton *btnLowStock = new QPushButton("🔴 Low Stock Only", dialog);
  btnLowStock->setStyleSheet("QPushButton { background: #fef3c7; color: "
                             "#92400e; border: 2px solid #f59e0b; "
                             "              border-radius: 8px; padding: 8px "
                             "12px; font-size: 13px; font-weight: bold; }"
...
void MainWindowController::AddToBookingInventoryClick() {
  ...
  if (currentRow < 0) {
    QMessageBox::warning(this, "Select Item",
                         "Please select an item from Inventory first!");
    return;
  }
  ...
  QLabel *titleLabel = new QLabel("Add Service to Booking", dialog);
  titleLabel->setStyleSheet("font-size: 20px; font-weight: bold; color: "
                            "#3730a3; margin-bottom: 15px;");
  mainLayout->addWidget(titleLabel, 0, Qt::AlignCenter);
  ...
  QString inputStyleStr =
      "QLineEdit, QComboBox, QSpinBox {"
      "   background-color: #ffffff; border: 2px solid #38bdf8; "
      "   border-radius: 8px; padding: 8px; font-size: 14px; "
      "   color: #0f172a; }"
      "QLineEdit:hover, QComboBox:hover, QSpinBox:hover { border-color: #0284c7; }"
      "QLineEdit:focus, QComboBox:focus, QSpinBox:focus { border-color: #0369a1; background-color: #f0f9ff; }"
      "QLineEdit[readOnly=\"true\"] { background-color: #e2e8f0; color: #475569; border-color: #cbd5e1; }"
      "QComboBox::drop-down { border: none; width: 25px; }"
      "QComboBox QAbstractItemView { background-color: #ffffff; color: #0f172a; border: 2px solid #38bdf8; selection-background-color: #f0f9ff; selection-color: #0369a1; }";

  QLineEdit *txtId = new QLineEdit(itemId, dialog);
  QLineEdit *txtName = new QLineEdit(itemName, dialog);
  QLineEdit *txtPrice = new QLineEdit(QString::number(itemPrice, 'f', 2), dialog);
  QComboBox *cbBookingId = new QComboBox(dialog);
  QSpinBox *spinQty = new QSpinBox(dialog);
  QLineEdit *txtNote = new QLineEdit(dialog);

  for (auto *input : std::initializer_list<QWidget*>{txtId, txtName, txtPrice, txtNote, cbBookingId, spinQty}) {
    input->setStyleSheet(inputStyleStr);
  }
  ...
  spinQty->setValue(1);
  txtNote->setPlaceholderText("Optional customer note");

  QSqlQuery bQuery(DatabaseManager::instance().database());
  if (bQuery.exec("SELECT id, room_number, status FROM Bookings WHERE status "
                  "IN ('CONFIRMED', 'CHECKED_IN') ORDER BY id DESC")) {
    while (bQuery.next()) {
      int bId = bQuery.value("id").toInt();
      QString rm = bQuery.value("room_number").toString();
      QString st = bQuery.value("status").toString();
      cbBookingId->addItem(QString("Booking #%1 — Room %2 (%3)")
                               .arg(bId)
                               .arg(rm.isEmpty() ? "-" : rm)
                               .arg(st.isEmpty() ? "CONFIRMED" : st),
                           bId);
    }
  }

  formLayout->addRow("Service ID:", txtId);
  formLayout->addRow("Service name:", txtName);
  formLayout->addRow("Final unit price:", txtPrice);
  formLayout->addRow("Booking:", cbBookingId);
  formLayout->addRow("Quantity:", spinQty);
  ...
  connect(btnSave, &QPushButton::clicked, [=]() {
    ...
    if (serviceRepo.addBookingServiceItem(itemData) != -1) {
      QMessageBox::information(
          dialog, "Success",
          "Added to Booking successfully and deducted from Inventory!");
      dialog->accept();
      showInventoryTab();
    } else {
