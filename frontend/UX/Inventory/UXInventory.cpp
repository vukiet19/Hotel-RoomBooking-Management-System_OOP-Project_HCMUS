...
void MainWindowController::showInventoryTab()
{
    ...
    QString inventoryQuery = "SELECT item_id AS 'Item ID', item_name AS 'Item Name', quantity AS 'Quantity', price AS 'Price ($)' FROM Inventory";
    ...
    if (btnAddToBooking) btnAddToBooking->setVisible(false);
...
void MainWindowController::AddInventoryClick()
{
    ...
    QString inputStyle = "QLineEdit, QSpinBox {"
                         ...
                         "QLineEdit:focus, QSpinBox:focus { border: 2px solid #0369a1; background-color: #f0f9ff; }";
    ...
    connect(btnSave, &QPushButton::clicked, [=]() {
        ...
        if (name.isEmpty() || price <= 0.0) {
            QMessageBox::warning(dialog, "Input Error", "Please input a valid Name and positive Price!");
            return;
        }

        InventoryRepository repo;
        if (repo.insertItem(name, qty, price)) {
            QMessageBox::information(dialog, "Success", "Added new item to inventory successfully!");
            dialog->accept();
            showInventoryTab();
        } else {
...
void MainWindowController::UpdateInventoryClick()
{
    ...
    int currentQty = tableInventory->item(currentRow, 2) ? tableInventory->item(currentRow, 2)->text().toInt() : 0;
    ...
    connect(btnSave, &QPushButton::clicked, [=]() {
        ...
        if (repo.updateQuantity(id.toInt(), spinQty->value())) {
            QMessageBox::information(dialog, "Success", "Item stock updated successfully!");
            dialog->accept();
            showInventoryTab();
        } else {
            QMessageBox::critical(dialog, "Error", "Failed to update stock quantity!");
        }
    });
...
void MainWindowController::DeleteInventoryClick()
{
    int currentRow = tableInventory->currentRow();
    if (currentRow < 0) {
        QMessageBox::warning(this, "Select Item", "Please select an item row to delete!");
        return;
    }
...
void MainWindowController::FilterInventoryClick()
{
    ...
    btnApply->setStyleSheet(
        "QPushButton { background: qlineargradient(x1:0, y1:0, x2:1, y2:0, stop:0 #10b981, stop:1 #059669); color: white; border: none; border-radius: 8px; padding: 10px 0; font-size: 15px; font-weight: bold; }"
    ...
    QString inventoryBaseQuery = "SELECT item_id AS 'Item ID', item_name AS 'Item Name', quantity AS 'Quantity', price AS 'Price ($)' FROM Inventory";
    ...
    connect(btnApply, &QPushButton::clicked, [=]() {
        QString queryStr = QString("SELECT item_id AS 'Item ID', item_name AS 'Item Name', quantity AS 'Quantity', price AS 'Price ($)' FROM Inventory WHERE quantity >= %1").arg(spinMinQty->value());

        if (!txtName->text().trimmed().isEmpty()) {
            queryStr += QString(" AND item_name LIKE '%%1%'").arg(txtName->text().trimmed());
        }
