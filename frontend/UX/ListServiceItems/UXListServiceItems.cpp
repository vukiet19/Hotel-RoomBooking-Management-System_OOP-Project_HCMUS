...
void MainWindowController::showServiceTab() {
  ...
  // Kéo dữ liệu từ Database, lấy tên cột là "Status" và chuyển 1/0 thành "Available/Unavailable"
  ...
  if (btnAddToBooking) btnAddToBooking->setVisible(true);
  ...
  connect(btnAdd, &QPushButton::clicked, this, &MainWindowController::showAddServiceDialog);
  connect(btnUpdate, &QPushButton::clicked, this, &MainWindowController::showUpdateServiceDialog);
  connect(btnDelete, &QPushButton::clicked, this, &MainWindowController::showDeleteServiceDialog);
  if (btnAddToBooking) {
    connect(btnAddToBooking, &QPushButton::clicked, this, &MainWindowController::showAddServiceToBookingDialog);
  }
  connect(btnFilter, &QPushButton::clicked, this, &MainWindowController::showFilterServiceDialog);
}
...
void MainWindowController::showAddServiceToBookingDialog() {
  ...
  QSqlQuery query(db);
  query.prepare("SELECT item_name, category, base_price, is_active FROM ServiceCatalog WHERE item_id = :id");
  ...
  if (query.exec() && query.next()) {
    ...
    QMessageBox::warning(this, "Service not found", "The selected service no longer exists in the catalog.");
    showServiceTab();
    return;
  }
  ...
  if (!isActive) {
      QMessageBox::critical(this, "Service Unavailable", 
                            "This service is currently marked as 'Unavailable' and cannot be added to a booking.");
      return;
  }
  ...
  connect(save, &QPushButton::clicked, &dialog,
          [this, &dialog, serviceId, serviceName, basePrice, booking, quantity, note, categoryStr] {
            ...
            if (!added) {
              if (categoryStr == "Food" || categoryStr == "Minibar" || categoryStr == "Furniture") {
                  InventoryService invService;
                  invService.releaseItem(serviceName, finalQty); // Hoàn lại kho nếu lưu lỗi
              }
...
void MainWindowController::showAddServiceDialog() {
  ...
  connect(cancel, &QPushButton::clicked, &dialog, &QDialog::reject);
  connect(save, &QPushButton::clicked, &dialog, [this, &dialog, id, name, category, price, statusCombo] {
    double basePrice = 0.0;
    if (id->text().trimmed().isEmpty() || name->text().trimmed().isEmpty()) {
      QMessageBox::warning(&dialog, "Invalid input", "Service ID and name are required.");
      return;
    }
    ...
    QSqlDatabase db = DatabaseManager::instance().database();
    QSqlQuery query(db);
    query.prepare("INSERT INTO ServiceCatalog (item_id, item_name, category, base_price, is_active) "
    ...
    if (!query.exec()) {
      QMessageBox::critical(&dialog, "Cannot add service", "The Service ID may already exist.");
      return;
    }
...
void MainWindowController::showUpdateServiceDialog() {
  if (!tableService || tableService->currentRow() < 0 ||
      !tableService->item(tableService->currentRow(), 0)) {
    QMessageBox::warning(this, "Select service", "Select a Service Catalog row to update.");
    return;
  }
  ...
  fetchQuery.prepare("SELECT item_name, category, base_price, is_active FROM ServiceCatalog WHERE item_id = :id");
  fetchQuery.bindValue(":id", itemId);
  
  if (!fetchQuery.exec() || !fetchQuery.next()) {
    QMessageBox::warning(this, "Service not found", "The selected service no longer exists.");
    showServiceTab();
    return;
  }
  ...
  connect(save, &QPushButton::clicked, &dialog, [this, &dialog, id, name, category, price, statusCombo] {
    ...
    QSqlDatabase db = DatabaseManager::instance().database();
    QSqlQuery query(db);
    query.prepare("UPDATE ServiceCatalog SET item_name = :name, category = :cat, base_price = :price, is_active = :status WHERE item_id = :id");
    query.bindValue(":id", id->text().trimmed());
    query.bindValue(":name", name->text().trimmed());
    query.bindValue(":cat", category->currentText());
    query.bindValue(":price", basePrice);
    ...
    if (!query.exec()) {
      QMessageBox::critical(&dialog, "Cannot update service", "The selected service could not be updated.");
      return;
    }
...
void MainWindowController::showDeleteServiceDialog() {
  if (!tableService || tableService->currentRow() < 0 ||
      !tableService->item(tableService->currentRow(), 0)) {
    QMessageBox::warning(this, "Select service", "Select a Service Catalog row to delete.");
    return;
  }
  ...
  checkQuery.prepare("SELECT 1 FROM BookingServiceItems WHERE item_id = :id LIMIT 1");
  ...
  if (!delQuery.exec()) {
    QMessageBox::critical(this, "Cannot delete service", "The service could not be deleted.");
    return;
  }
...
void MainWindowController::showFilterServiceDialog() {
  ...
  auto *name = new QLineEdit(&dialog);
  auto *category = new QComboBox(&dialog);
  category->addItems({"All", "Food", "Minibar", "Furniture", "Facility", "Damage"});
  category->setStyleSheet(inputStyle());
  ...
  connect(apply, &QPushButton::clicked, &dialog,
          [this, &dialog, id, name, category, minPrice, maxPrice, statusCombo] {
            ...
            if (minValue >= 0.0 && maxValue >= 0.0 && minValue > maxValue) {
              QMessageBox::warning(&dialog, "Invalid input", "Minimum price cannot exceed maximum price.");
              return;
            }
            ...
            if (!name->text().trimmed().isEmpty()) {
                queryStr += QString(" AND item_name LIKE '%%1%'").arg(name->text().trimmed());
            }
            if (category->currentText() != "All") {
                queryStr += QString(" AND category = '%1'").arg(category->currentText());
            }
            if (minValue >= 0.0) {
