...
void MainWindowController::showBookingTab()
{
    ...
    connect(btnUpdate, &QPushButton::clicked, this, &MainWindowController::showUpdateBookingDialog);
    connect(btnDelete, &QPushButton::clicked, this, &MainWindowController::showDeleteBookingDialog);
...
void MainWindowController::showUpdateBookingDialog()
{
    ...
    QString inputStyle =
        ...
        "QLineEdit:focus, QComboBox:focus, QDateEdit:focus { border: 2px solid #0369a1; background-color: #f0f9ff; }"
        "QComboBox::drop-down { border: none; width: 25px; }"
        "QComboBox::down-arrow { image: none; }"
        "QComboBox QAbstractItemView, QComboBox QListView {"
        "   background-color: #ffffff; "
    ...
    QDateEdit *dateCheckOut = new QDateEdit(QDate::currentDate().addDays(1), dialog);
    ...
    if (tableBooking && tableBooking->currentRow() >= 0)
    {
        ...
        if (tableBooking->item(row, 0)) txtBookingId->setText(tableBooking->item(row, 0)->text());
        if (tableBooking->item(row, 1)) txtCustomerId->setText(tableBooking->item(row, 1)->text());
        if (tableBooking->item(row, 2)) txtRoomNumber->setText(tableBooking->item(row, 2)->text());
        if (tableBooking->item(row, 3)) {
            QDate inD = QDate::fromString(tableBooking->item(row, 3)->text().left(10), "yyyy-MM-dd");
            if (inD.isValid()) dateCheckIn->setDate(inD);
        }
        if (tableBooking->item(row, 4)) {
            QDate outD = QDate::fromString(tableBooking->item(row, 4)->text().left(10), "yyyy-MM-dd");
            if (outD.isValid()) dateCheckOut->setDate(outD);
        }
        if (tableBooking->item(row, 5)) txtPrice->setText(tableBooking->item(row, 5)->text());
    }
    ...
    connect(btnSave, &QPushButton::clicked, [=]()
    {
        ...
        if (bIdStr.isEmpty()) {
            QMessageBox::warning(dialog, "Input Error", "Booking ID is required to update a booking.");
            return;
        }
        ...
        if (!okId || bookingId <= 0) {
            QMessageBox::warning(dialog, "Input Error", "Booking ID must be a positive integer.");
            return;
        }
        ...
        QString checkInStr = dateCheckIn->date().toString("yyyy-MM-dd") + " 14:00:00";
        QString checkOutStr = dateCheckOut->date().toString("yyyy-MM-dd") + " 12:00:00";
        QString statusStr = cbStatus->currentText();
        ...
        bool success = repo.updateBooking(bookingId, customerIdStr.toInt(), roomNumberStr, inDT, outDT, priceVal, statusStr);

        if (success) {
            QMessageBox::information(dialog, "Success", "Booking updated successfully!");
            Backend::loadTableData(tableBooking, "SELECT id AS 'Booking ID', customer_id AS 'Customer ID', room_number AS 'Room Number', check_in_time AS 'Check-In', check_out_time AS 'Check-Out', total_price AS 'Total Price ($)' FROM Bookings");
            dialog->accept();
        } else {
            QMessageBox::critical(dialog, "Error", "Failed to update Booking. Please verify the Booking ID exists.");
        }
    });
...
void MainWindowController::showDeleteBookingDialog()
{
    ...
    connect(btnDelete, &QPushButton::clicked, dialog, [=]()
    {
        ...
        if (!ok || bookingId <= 0) {
            QMessageBox::warning(dialog, "Error", "Booking ID must be a valid integer!");
            return;
        }

        QMessageBox::StandardButton reply = QMessageBox::question(dialog, "Confirm", QString("Are you sure you want to delete Booking ID %1?").arg(bookingId), QMessageBox::Yes | QMessageBox::No);
        ...
        if (success) {
            QMessageBox::information(dialog, "Success", "Booking deleted successfully!");
            dialog->accept();
            Backend::loadTableData(tableBooking, "SELECT id AS 'Booking ID', customer_id AS 'Customer ID', room_number AS 'Room Number', check_in_time AS 'Check-In', check_out_time AS 'Check-Out', total_price AS 'Total Price ($)' FROM Bookings");
            Backend::loadTableData(tableRoom, "SELECT * FROM ListRooms");
        } else {
            QMessageBox::critical(dialog, "Error", "Cannot delete booking. It may not exist in the database.");
        }
    });
