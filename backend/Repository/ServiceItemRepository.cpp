...
vector<ServiceCatalogData> ServiceItemRepository::getAllCatalogItems() {
  ...
  if (!query.exec()) {
    qDebug() << "ERROR: Khong the doc ServiceCatalog!"
...
*/
optional<ServiceCatalogData>
ServiceItemRepository::findCatalogItemById(const string &itemId) {
  QSqlDatabase db = DatabaseManager::instance().database();
  QSqlQuery query(db);
  ...
  if (!query.exec()) {
    qDebug() << "ERROR: Khong the tim ServiceCatalog item!"
             << query.lastError().text();
...
/*
hàm này dùng để lọc các dịch vụ có trong menu ServiceCatalog theo nhiều điều
kiện khác nhau Nếu một field trong filter mang giá trị mặc định thì mình bỏ qua
điều kiện đó
...
*/
vector<ServiceCatalogData> ServiceItemRepository::getFilteredCatalogItems(
    const ServiceCatalogFilter &filter) {
  vector<ServiceCatalogData> items;
  ...
  if (!filter.name.empty()) {
    query.bindValue(":item_name",
                    QString("%") + QString::fromStdString(filter.name) + "%");
  }
  if (!filter.category.empty()) {
  ...
  if (!query.exec()) {
    qDebug() << "ERROR: Khong the loc ServiceCatalog!"
             << query.lastError().text();
    return items;
  }
...
bool ServiceItemRepository::addCatalogItem(const ServiceCatalogData &item) {
  if (item.id.empty() || item.name.empty() || item.category.empty() ||
      item.basePrice < 0.0) {
    return false;
  }
  ...
  query.prepare(
      "INSERT INTO ServiceCatalog (item_id, item_name, category, base_price, "
      "is_active) "
  ...
  if (!query.exec()) {
    qDebug() << "ERROR: Khong the them ServiceCatalog item!"
             << query.lastError().text();
    return false;
  }
  return true;
...
bool ServiceItemRepository::updateCatalogItem(const ServiceCatalogData &item) {
  ...
  QSqlQuery query(DatabaseManager::instance().database());
  query.prepare("UPDATE ServiceCatalog "
                "SET item_name = :item_name, category = :category, base_price "
                "= :base_price, "
                "is_active = :is_active "
                "WHERE item_id = :item_id");
  query.bindValue(":item_id", QString::fromStdString(item.id));
  ...
  if (!query.exec()) {
    qDebug() << "ERROR: Khong the cap nhat ServiceCatalog item!"
             << query.lastError().text();
    return false;
  }
...
bool ServiceItemRepository::isCatalogItemInUse(const string &itemId) {
  QSqlQuery query(DatabaseManager::instance().database());
  query.prepare(
      "SELECT 1 FROM BookingServiceItems WHERE item_id = :item_id LIMIT 1");
  query.bindValue(":item_id", QString::fromStdString(itemId));

  if (!query.exec()) {
    qDebug() << "ERROR: Khong the kiem tra ServiceCatalog item!"
             << query.lastError().text();
    return true;
  }
  return query.next();
...
bool ServiceItemRepository::removeCatalogItem(const string &itemId) {
  ...
  if (!query.exec()) {
    qDebug() << "ERROR: Khong the xoa ServiceCatalog item!"
             << query.lastError().text();
    return false;
  }
  return query.numRowsAffected() > 0;
...
*/
int ServiceItemRepository::addBookingServiceItem(
    const BookingServiceItemData &item) {
  QSqlDatabase db = DatabaseManager::instance().database();
  QSqlQuery query(db);

  query.prepare("INSERT INTO BookingServiceItems "
  ...
  if (!query.exec()) {
    qDebug() << "ERROR: Khong the them BookingServiceItem!"
             << query.lastError().text();
  ...
  // Recalculate and update live Bookings.total_price (room charge + services -
  // deposit)
  QSqlQuery bQuery(db);
  bQuery.prepare(R"(
        SELECT b.room_number, b.check_in_time, b.check_out_time, b.deposit_amount, b.deposit_status, r.base_price
        FROM Bookings b
        LEFT JOIN ListRooms r ON b.room_number = r.room_number OR b.room_number = r.room_id
        WHERE b.id = :bId
    )");
  bQuery.bindValue(":bId", item.bookingId);
  if (bQuery.exec() && bQuery.next()) {
    QString rmNum = bQuery.value(0).toString();
    QDateTime inDT =
        QDateTime::fromString(bQuery.value(1).toString(), Qt::ISODate);
    if (!inDT.isValid())
      inDT = QDateTime::fromString(bQuery.value(1).toString(),
                                   "yyyy-MM-dd hh:mm:ss");
    QDateTime outDT =
        QDateTime::fromString(bQuery.value(2).toString(), Qt::ISODate);
    if (!outDT.isValid())
      outDT = QDateTime::fromString(bQuery.value(2).toString(),
                                    "yyyy-MM-dd hh:mm:ss");
    double deposit = (bQuery.value(4).toString() == "HELD")
                         ? bQuery.value(3).toDouble()
                         : 0.0;
    double baseP = bQuery.value(5).toDouble();
    int nights = inDT.daysTo(outDT);
    if (nights <= 0)
      nights = 1;

    QSqlQuery sQuery(db);
    sQuery.prepare("SELECT SUM(final_price) FROM BookingServiceItems WHERE "
                   "booking_id = :bId");
    sQuery.bindValue(":bId", item.bookingId);
    double sTotal = 0.0;
    ...
    uQuery.bindValue(":bId", item.bookingId);
    uQuery.exec();
  }
...
*/
vector<BookingServiceItemData>
ServiceItemRepository::getItemsByBookingId(int bookingId) {
  ...
  if (!query.exec()) {
    qDebug() << "ERROR: Khong the doc BookingServiceItems!"
             << query.lastError().text();
    return items;
  }
...
/*
hàm này dùng để lọc các dịch vụ đã được thêm vào một booking trong bảng
BookingServiceItems Nó khác ServiceCatalog ở chỗ bảng này lưu dịch vụ phát sinh
thực tế của khách, nên filter sẽ dựa trên booking_id, item_id, quantity,
final_price và customer_note

nếu một field trong BookingServiceItemFilter mang giá trị mặc định thì mình bỏ
qua điều kiện đó Ví dụ bookingId = -1 thì không lọc theo booking_id, itemId rỗng
thì không lọc theo item_id, minQuantity/maxQuantity = -1 thì không lọc theo số
lượng, minFinalPrice/maxFinalPrice < 0 thì không lọc theo tiền cuối cùng
...
*/
vector<BookingServiceItemData>
ServiceItemRepository::getFilteredBookingServiceItems(
    const BookingServiceItemFilter &filter) {
  vector<BookingServiceItemData> items;
  ...
  if (!filter.customerNote.empty()) {
    query.bindValue(":customer_note",
                    QString("%") + QString::fromStdString(filter.customerNote) +
                        "%");
  }

  if (!query.exec()) {
    qDebug() << "ERROR: Khong the loc BookingServiceItems!"
             << query.lastError().text();
    return items;
  }
...
bool ServiceItemRepository::removeBookingServiceItem(int id) {
  ...
  query.prepare("DELETE FROM BookingServiceItems "
                "WHERE id = :id");

  query.bindValue(":id", id);

  if (!query.exec()) {
    qDebug() << "ERROR: Khong the xoa BookingServiceItem!"
             << query.lastError().text();
    return false;
  }
...
bool ServiceItemRepository::removeItemsByBookingId(int bookingId) {
  ...
  if (!query.exec()) {
    qDebug() << "ERROR: Khong the xoa BookingServiceItems theo booking_id!"
             << query.lastError().text();
    return false;
  }
