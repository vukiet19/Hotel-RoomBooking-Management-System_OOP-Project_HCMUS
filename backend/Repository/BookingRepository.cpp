...
bool BookingRepository::add(const BookingData &booking) {
  ...
  double priceToSave = booking.totalPrice;
  if (priceToSave <= 0.0) {
    RoomRepository roomRepo;
    Room *r = roomRepo.getById(booking.roomNumber.toStdString());
    if (r) {
      priceToSave = r->getBasePrice();
      delete r;
    } else {
      QSqlQuery rq(db);
      rq.prepare("SELECT base_price FROM ListRooms WHERE room_number = :rm OR "
                 "room_id = :rm");
      rq.bindValue(":rm", booking.roomNumber);
      if (rq.exec() && rq.next()) {
        priceToSave = rq.value(0).toDouble();
      }
    }
  }
  ...
  query.prepare("INSERT INTO Bookings (customer_id, room_number, "
                ...
                ":totalPrice, 'STANDARD', 'UNCONFIRMED', 0.0, 'NONE')");
  ...
  query.bindValue(":totalPrice", priceToSave);
